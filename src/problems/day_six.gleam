...
pub type MapState {
  MapState(map: Dict(#(Int, Int), String), guard: #(#(Int, Int), String))
}
...
fn parse_input_to_map_state(input: String) -> MapState {
  ...
  |> list.index_fold(
    from: MapState(dict.new(), #(#(0, 0), "")),
    with: fn(map_state, str, x) {
      str
      |> string.to_graphemes
      |> list.index_fold(from: map_state, with: fn(map_state, char, y) {
        case is_guard(char) {
          True ->
            MapState(dict.insert(map_state.map, #(x, y), char), #(#(x, y), char))
          False ->
            MapState(dict.insert(map_state.map, #(x, y), char), map_state.guard)
        }
      })
    },
  )
...
fn find_path_to_exit(map_state: MapState) -> MapState {
  ...
  case result.is_ok(next_postion_in_dict) {
    True ->
      map_state
      |> execute_action_on_next_position(#(
        next_position,
        result.unwrap(next_postion_in_dict, ""),
      ))
      |> find_path_to_exit
    False ->
      map_state.map
      |> dict.upsert(map_state.guard.0, fn(_op) { "X" })
      |> MapState(map_state.guard)
  }
}
...
fn execute_action_on_next_position(
  map_state: MapState,
  next: #(#(Int, Int), String),
) -> MapState {
  case next {
    #(#(_x, _y), ".") | #(#(_x, _y), "X") ->
      map_state.map
      |> dict.upsert(next.0, fn(_op) { map_state.guard.1 })
      |> dict.upsert(map_state.guard.0, fn(_op) { "X" })
    ...
    #(#(_x, _y), "#") -> {
      ...
      map_state.map
      |> dict.upsert(map_state.guard.0, fn(_op) { new_guard_orientation })
...
fn map_state_to_string(map_state: MapState) -> String {
  ...
  |> list.map(transform_to_lines(
    get_dict_from_map_state(map_state)
      |> dict.to_list
      |> list.sort(by: sort_map_entry),
    _,
  ))
  |> string.join("\n")
}
