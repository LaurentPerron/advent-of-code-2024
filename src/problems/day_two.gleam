...
fn is_report_safe(
  direction: fn(Int, Int) -> Bool,
  report: List(Int),
  is_safe: Bool,
) -> Bool {
  case report, is_safe {
    [first, second, ..rest], True ->
      is_report_safe(
        direction,
        list.append([second], rest),
        is_variation_safe(first, second, direction),
      )
    [_], True -> True
    [], True -> True
    _, False -> False
...
fn is_variation_safe(
  current: Int,
  next: Int,
  direction: fn(Int, Int) -> Bool,
) -> Bool {
  direction(current, next)
  && int.absolute_value(current - next) <= maximum_deviation
}
...
fn get_initial_direction(report: List(Int)) -> fn(Int, Int) -> Bool {
  case report {
    [first, second, ..] if first >= second -> fn(current, next) {
      current > next
    }
    [first, second, ..] if first < second -> fn(current, next) {
      current < next
